...
def cross_validation(train, regr_func, hyperparams, loss_func, num_folds=10, verbose=False):
...
"""
Out-of-bag ensemble validation

This function takes and already trained ensemble with k total estimators and also
a test set.

The function predicts the test set using differing numbers of estimators from
min_estimators to max_estimators at user specified estimators. The number of est.
which produced the smallest error is used as the final number of estimators.
"""
def OOB_ensemble_validation(train, ensemble, min_estimators=10, interval=5, p=0.3, num_iter=10):
    # Determine the largest number of estimators in the ensemble
    max_estimators = ensemble.num_estimators
    # Get the list of numbers of estimators to be tried
    to_try = np.array(range(min_estimators, max_estimators + interval, interval))
    
    # Build an array to store the training err for each hyperparameter
    # pair for all folds
    test_losses = np.zeros((len(to_try), num_iter))
    
    print '-------------------------------------------------------------'
    print 'Beginning OOB Ensemble Validation.'
    print '-------------------------------------------------------------'
    
    for f in range(0, num_iter):
        # Shuffle the training data
        np.random.shuffle(train)
        # Select the first p% of the the training data
        inds = np.arange( int(len(train) * p) )
        temp = train[inds]
        
        # Break the data apart into components
        test_x = temp[:,:-1]
        test_y = temp[:,-1]

        # Test the different number of estimators on the test set
        prev_predictions = []
        for j in range(0, len(to_try)):
            # Base case - just use min_estimators
            if (j == 0):
                predictions = ensemble.predict(test_x, n=to_try[j], upto_n=True)
                test_losses[j, f] = mean_squared_error(predictions, test_y)
                prev_predictions = predictions
            # Else, the prediction of the ensemble using to_try[j] estimators
            # will be the product of the predictions of the ensemble using to_try[j-1]
            # estimators and to_try[j-1] + the predictions of each estimator gained
            else:
                predictions = prev_predictions
                # Get the previous predictions times the number of prev estimators
                test_losses[j, f] = test_losses[j-1,f] * to_try[j-1]
                # Add the prediction of each new estimator
                for s in range(to_try[j-1] + 1, to_try[j] + 1):
                    predictions += ensemble.predict(test_x, n=to_try[j-1] + s)
                # Divide by the total number of estimators to get the mean
                test_losses[j,f] /= to_try[j]
                # Now get the loss
                test_losses[j, f] = mean_squared_error(predictions, test_y)
                # update the previous predictions
                prev_predictions = predictions
        # to alert the user of progress        
        print str(f + 1) + ' iterations completed.'
    
    mean_loss = np.mean(test_losses, axis=1)
    # Get the index of the best number of estimators
    best_n_ind = np.argmin( mean_loss )
    # Get the loss of the best number of estimators
    min_loss = mean_loss[ best_n_ind ]
    
    # Print to stdout the results.
    print 'OOB ensemble validation completed.'
    print 'Loss on the best hyperparameter setting: ' + str(min_loss)
    return to_try[ best_n_ind ]

    

# Train-Test Validation
# Note: This is NOT a hyperparameter optimization algorithm.
# This function just assesses the generalization performance of the single
#   set of selected features and a hyperparameter setting.
# This is more useful to the genetic feature/hyperparameter selection algorithm.
# Additionally, this is faster than a cross-validation, and thus it was used for 
#   the genetic programming. 
def train_test_validation(train, regr_func, hparams, feat_set, loss_func, split=0.8, verbose=0):
    # Calculate the number of elements for training
    Tr = int(split * len(train))
    # Shuffle the training data
    np.random.shuffle(train)
    # Fit the model for each hyperparam setting
    # and record the loss
    # Split the training data into features and classification,
    # applying the passed features
    train_x = train[:Tr, 0:-1][:, feat_set]
    train_y = train[:Tr, -1]
    test_x = train[Tr:, 1:][:, feat_set]
    test_y = train[Tr:, -1]
    
    # Train the model on the training set using its passed hyperparameters
    regr = regr_func(hparams).fit(train_x, train_y)
    # Return the loss for this hyperparameter setting
    return (regr, calculate_loss(loss_func, regr, test_x, test_y))
